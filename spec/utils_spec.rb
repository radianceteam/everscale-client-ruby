...
describe EverSdk::Utils do
  context "methods of utils" do
    it "#convert_address" do
      ...
      # 1
      pr1 = EverSdk::Utils::ParamsOfConvertAddress.new(
        address: account_id,
        output_format: EverSdk::Utils::AddressStringFormat.new(type_: :hex)
      )

      expect { |b| EverSdk::Utils.convert_address(@c_ctx.context, pr1, &b) }.to yield_control
      expect { |b| EverSdk::Utils.convert_address(@c_ctx.context, pr1, &b) }.to yield_with_args(EverSdk::NativeLibResponseResult)

      EverSdk::Utils.convert_address(@c_ctx.context, pr1) { |a| @res1 = a }
      expect(@res1.success?).to eq true
      expect(@res1.result.address).to eq hex_workchain0


      # 2
      pr2 = EverSdk::Utils::ParamsOfConvertAddress.new(
        address: account_id,
        output_format: EverSdk::Utils::AddressStringFormat.new(type_: :account_id)
      )

      expect { |b| EverSdk::Utils.convert_address(@c_ctx.context, pr2, &b) }.to yield_control
      expect { |b| EverSdk::Utils.convert_address(@c_ctx.context, pr2, &b) }.to yield_with_args(EverSdk::NativeLibResponseResult)

      EverSdk::Utils.convert_address(@c_ctx.context, pr2) { |a| @res2 = a }
      expect(@res2.success?).to eq true
      expect(@res2.result.address).to eq account_id

      # 3
      pr3 = EverSdk::Utils::ParamsOfConvertAddress.new(
        address: hex,
        output_format: EverSdk::Utils::AddressStringFormat.new(
          type_: :base64,
          bounce: false,
          test_: false,
          url: false
      ...
      expect { |b| EverSdk::Utils.convert_address(@c_ctx.context, pr3, &b) }.to yield_control
      expect { |b| EverSdk::Utils.convert_address(@c_ctx.context, pr3, &b) }.to yield_with_args(EverSdk::NativeLibResponseResult)

      EverSdk::Utils.convert_address(@c_ctx.context, pr3) { |a| @res3 = a }
      expect(@res3.success?).to eq true
      expect(@res3.result.address).to eq base64

      # 4
      pr4 = EverSdk::Utils::ParamsOfConvertAddress.new(
        address: base64,
        output_format: EverSdk::Utils::AddressStringFormat.new(
          type_: :base64,
          bounce: true,
      ...
      expect { |b| EverSdk::Utils.convert_address(@c_ctx.context, pr4, &b) }.to yield_control
      expect { |b| EverSdk::Utils.convert_address(@c_ctx.context, pr4, &b) }.to yield_with_args(EverSdk::NativeLibResponseResult)

      EverSdk::Utils.convert_address(@c_ctx.context, pr4) { |a| @res4 = a }
      expect(@res4.success?).to eq true
      expect(@res4.result.address).to eq base64url

      # 5
      pr5 = EverSdk::Utils::ParamsOfConvertAddress.new(
        address: base64,
        output_format: EverSdk::Utils::AddressStringFormat.new(type_: :hex)
      )
      EverSdk::Utils.convert_address(@c_ctx.context, pr5) { |a| @res5 = a }
    ...
    it "#calc_storage_fee" do
      ...
      params = EverSdk::Utils::ParamsOfCalcStorageFee.new(
        account: account,
        period: 1000
      )
      EverSdk::Utils.calc_storage_fee(@c_ctx.context, params) { |r| @response = r }
    ...
    it "#compression" do
      ...
      params = EverSdk::Utils::ParamsOfCompressZstd.new(
        uncompressed: Base64.strict_encode64(uncompressed),
        level: 21
      )
      EverSdk::Utils.compress_zstd(@c_ctx.context, params) { |r| @response = r }
      ...
      params = EverSdk::Utils::ParamsOfDecompressZstd.new(compressed: @response.result.compressed)
      EverSdk::Utils.decompress_zstd(@c_ctx.context, params) { |r| @response = r }
    ...
    it "#get_address_type" do
      ["", "   ", "123456", "abcdef"].each do |address|
        params = EverSdk::Utils::ParamsOfGetAddressType.new(address: address)
        EverSdk::Utils.get_address_type(@c_ctx.context, params) { |r| @response = r }
